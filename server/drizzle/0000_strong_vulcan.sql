CREATE TABLE `organization_contexts` (
	...
	CONSTRAINT `organization_contexts_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `organization_llms` (
	`id` varchar(36) NOT NULL DEFAULT (uuid()),
	`organization_id` varchar(36) NOT NULL,
	...
	`updated_at` timestamp NOT NULL DEFAULT (now()),
	`created_at` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `organization_llms_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
--> statement-breakpoint
ALTER TABLE `organization_contexts` ADD CONSTRAINT `organization_contexts_updated_by_users_id_fk` FOREIGN KEY (`updated_by`) REFERENCES `users`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `organization_llms` ADD CONSTRAINT `organization_llms_organization_id_organizations_id_fk` FOREIGN KEY (`organization_id`) REFERENCES `organizations`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
